...
//setting workspace can read
async function workspace_canwrite(workspace){
  var xmlDom = Blockly.Xml.workspaceToDom(workspace);
  workspace.dispose();
  workspace = Blockly.inject('content_area1',
    {
...
function student_tab_handler(clickedName) { 
  ...
  var xml = Blockly.Xml.textToDom(window.localStorage.getItem(selected));
  other_workspace.clear();
  Blockly.Events.disable();
  Blockly.Xml.domToWorkspace(xml, other_workspace);
  Blockly.Events.enable();
...
function discard() {
  ...
  if (count < 2 ||
    ...
    Blockly.Event.disable();
    my_workspace.clear();
    Blockly.Event.enable();
